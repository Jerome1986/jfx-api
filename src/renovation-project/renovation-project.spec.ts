...
describe('装修项目确认与详情', () => {
  ...
  const project = { id: 1, userId: 7, status: 'PENDING_CONFIRM', updatedAt: new Date(), quotedAmount: new Prisma.Decimal('123.45') }
  ...
  it('详情查询限定登录用户，不存在或不属于本人时返回 404', async () => {
    ...
    await expect(service.confirmProject(1, user)).rejects.toBeInstanceOf(NotFoundException)
  ...
  it.each([0, -1, NaN, 1.5, Number.MAX_SAFE_INTEGER + 1])('拒绝非法 ID %s', async (id) => {
    await expect(service.confirmProject(id, user)).rejects.toBeInstanceOf(BadRequestException)
  ...
  it.each(['CANCELED', 'COMPLETED', 'PENDING_QUOTE', 'IN_SERVICE'])('拒绝状态 %s，重复确认不再写入', async (status) => {
    ...
    await expect(service.confirmProject(1, user)).rejects.toBeInstanceOf(ConflictException)
    expect(repo.confirmProject).not.toHaveBeenCalled()
  })

  it('确认成功返回服务中项目及成交金额', async () => {
    await expect(service.confirmProject(1, user)).resolves.toMatchObject({ status: 'IN_SERVICE', contractAmount: project.quotedAmount })
  })
  ...
  it('不把其他数据库故障误报为状态冲突', async () => {
    ...
    await expect(service.confirmProject(1, user)).rejects.toBe(error)
  })
  ...
  it('数据库写入同时约束归属、状态及报价快照，原子保存成交金额', async () => {
    ...
    expect(update).toHaveBeenCalledWith({
      where: { id: 1, userId: 7, status: 'PENDING_CONFIRM', updatedAt: project.updatedAt, quotedAmount: project.quotedAmount },
